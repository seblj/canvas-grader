...
async fn get_assignments(section_ids: &[i32]) -> Result<Vec<Assignment>, anyhow::Error> {
    let mut contents: Vec<Assignment> = Vec::new();
    let assignment = dotenv::var("ASSIGNMENT").expect("Couldn't find id for assignment");
    for section_id in section_ids.iter() {
        let mut url = format!(
            "sections/{section_id}/assignments/{assignment}/submissions?per_page=100&include=user"
        );
        loop {
            let res = get(&url).await?;
            let links = parse(res.headers().get("link").unwrap().to_str()?)?;

            let mut body = res.json::<Vec<Assignment>>().await?;
            contents.append(&mut body);

            if let Some(last) = links.get(&Some("last".to_string())) {
                if let Some(current) = links.get(&Some("current".to_string())) {
                    if current.uri == last.uri {
                        break;
                    }
                }
            }
            if let Some(next) = links.get(&Some("next".to_string())) {
    ...
    for assignment in assignments.iter() {
        ...
        if let Some(ref attachments) = assignment.attachments {
            ...
            for attachment in attachments.iter() {
                match download_file(&attachment.url, &attachment.filename, Path::new(&user_dir))
                    .await
                {
                    Ok(_) => (),
                    Err(e) => {
...
async fn get_section_ids() -> Result<Vec<i32>, anyhow::Error> {
    let url = format!("users/self/courses?include=sections");
    let res = get(&url).await?;
    ...
    return Ok(course
        ...
        .iter()
        .filter_map(|section| {
            if section.enrollment_role == EnrollmentRole::TaEnrollment
                || section.enrollment_role == EnrollmentRole::TeacherEnrollment
